...
public class GraphQueryEntityExtractor {
    ...
    private static final long EXTRACT_TIMEOUT_MS = 8000;
    ...
    public List<String> extract(String query, String kbId) {
        ...
        try {
            ...
            String output = CompletableFuture.supplyAsync(() ->
                            llmService.chat(req, config.getExtractModelId(), config.getExtractModelName()))
                    .orTimeout(EXTRACT_TIMEOUT_MS, TimeUnit.MILLISECONDS)
                    .join();
            ...
            if (cause instanceof TimeoutException || e instanceof TimeoutException) {
                log.warn("[QueryEntity] LLM 抽取超时 {}ms，本轮跳过图谱 query={}", EXTRACT_TIMEOUT_MS, query);
            } else {
                log.warn("[QueryEntity] LLM 抽取失败: {}", cause.getMessage());
            }
            return List.of();
        }
    }
