...
public class RerankStage implements PipelineStage {
    ...
    public StageResult execute(PipelineContext ctx) throws Exception {
        ...
        // 1. 批量打分
        //    指定 rerankModelId → 真实 rerank；有知识库 → 库 embedding 余弦；都没有 → 原 ScoringModel 兜底。
        ...
        if (rerankModelId != null) {
            try {
                ...
                log.warn("[Rerank] 智能体重排模型(id={})调用失败，回退知识库 embedding 余弦: {}",
                        rerankModelId, e.getMessage());
                scores = null;
            }
        }
        if (scores == null) {
            String kbId = (ctx.getKnowledgeBaseIds() == null || ctx.getKnowledgeBaseIds().isEmpty())
                    ? null : ctx.getKnowledgeBaseIds().get(0);
            if (kbId != null) {
                // 主路径：与检索同一知识库 embedding
                try {
                    EmbeddingModel emb = embeddingModelProvider.resolve(kbId);
                    scores = new CohereScoringModelConfig.EmbeddingBasedScoringModel(emb)
                            .scoreAll(passages, query).content();
                } catch (Exception e) {
                    log.warn("[Rerank] 知识库 embedding 余弦打分失败，回退全局 ScoringModel: {}", e.getMessage());
                    scores = null;
                }
            }
            if (scores == null) {
