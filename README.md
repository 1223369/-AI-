...
本项目是一个采用 **大语言模型 + 可视化编排** 构建的企业级 AI 智能体开发平台，覆盖从知识库入库、检索增强、智能体对话到工作流编排的完整链路。开箱即用、模型任选、灵活编排。
...
| 💡 | <a href="#-为什么需要" target="_blank">为什么需要</a> | 设计思考与价值 |
| 🏗️ | <a href="#-核心设计" target="_blank">核心设计</a> | 架构与工程实践 |
...
本项目的做法是：**多候选路由 + 三态熔断 + 自动降级**。一个模型挂了，自动切到下一个候选，配合首包探测保证切换对用户无感知。
...
本项目提供了**可视化工作流编排**，把多个节点串成图，让 AI 按流程协同工作，而不是只会一问一答。
...
生产环境不可能只依赖一个模型供应商。容错层解决的就是"模型不稳定怎么办"：
