<template>
  <div class="chat-app" :class="{ 'has-session': !!activeSessionId }">
    ...
    <div v-if="!activeSessionId" class="chat-landing">
      <div class="cc-container">
        ...
        <h1 class="cc-greeting">
          {{ selectedAgentWelcome || 'Hi，让你的知识触手可及' }}
        </h1>
    ...
    <template v-else>
      ...
      <LiveStagePanel
        v-if="liveStages.length || streaming"
        :items="liveStages"
        :running="streaming && currentKind === 'agent'"
      />
    </template>
...
<script setup lang="ts">
  ...
  const liveStages = ref<LiveStageItem[]>([]);
  ...
  function applyStageEvent(event: {
    ...
    const list = liveStages.value;
    const idx = list.findIndex((s) => s.name === event.name);
    if (event.type === 'stage') {
      const item: LiveStageItem = {
        name: event.name,
        label: event.label,
        status: 'running',
      };
