...
/** 实时阶段进度（管线边跑边推 SSE stage / stage_end / stage_skip） */
export interface AgentChatStageEvent {
  type: 'stage' | 'stage_end' | 'stage_skip';
  name: string;
  label: string;
  costMs?: number;
  result?: string;
...
export type LiveStageStatus = 'running' | 'done' | 'skipped' | 'error';
...
export async function streamAgentChat(
  ...
  handlers: {
    ...
    onStage?: (event: AgentChatStageEvent) => void;
  },
  signal?: AbortSignal,
): Promise<void> {
...
function parseSseFrame(
  ...
  if (eventName === 'answer' || payload.type === 'answer') {
    ...
    eventName === 'stage' ||
    eventName === 'stage_end' ||
    eventName === 'stage_skip' ||
    payload.type === 'stage' ||
    payload.type === 'stage_end' ||
    payload.type === 'stage_skip'
  ) {
    handlers.onStage?.({
      type: payload.type || eventName,
      name: payload.name || '',
      label: payload.label || payload.name || '',
      costMs: typeof payload.costMs === 'number' ? payload.costMs : undefined,
